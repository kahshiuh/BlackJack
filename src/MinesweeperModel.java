...
public class MinesweeperModel {
    ...
    public void setBombs() {
        ...
        for (int i = 0; i < mines; i++) {
            ...
            //Makes sure there is not already a bomb there and is no in in 3*3 radius of start
    ...
    public int checkSurrounding(int r, int c) {
        ...
        for (int i = -1; i < 2; i++) {
            for (int j = -1; j < 2; j++) {
                if (r + i > -1 && r + i < size && c + j > -1 && c + j < size && board[r + i][c + j].getMineStatus() == true) {
                    count++;
                }
            }
        }
    ...
    //Should be called to uncover all surrounding zeros (In terms of neighboring bombs)
    //Uncovers
    ...
    public void uCS(int r, int c) {
        ...
        if (board[r][c].getSurroundingBombs() == 0) {
            //Down one
            if (r+1<size && board[r + 1][c].getRevealedStatus() == false && board[r + 1][c].getSurroundingBombs() == 0) {
                uCS(r + 1, c);
            }
            //Up one
            if (r - 1 >= 0 && board[r - 1][c].getRevealedStatus() == false && board[r - 1][c].getSurroundingBombs() == 0) {
                uCS(r - 1, c);
            }
            //Right one
            if (c + 1 < size && board[r][c + 1].getRevealedStatus() == false && board[r][c + 1].getSurroundingBombs() == 0) {
                uCS(r, c + 1);
            }
            //Left One
            if (c - 1 >= 0 && board[r][c - 1].getRevealedStatus() == false && board[r][c - 1].getSurroundingBombs() == 0) {
                uCS(r, c - 1);
            }
            if(r+1 < size && c+1 < size && board[r+1][c+1].getRevealedStatus() == false && board[r+1][c+1].getSurroundingBombs() == 0){
                uCS(r+1,c+1);
                
            }
            if(r+1 < size && c-1 >=0 && board[r+1][c-1].getRevealedStatus() == false && board[r+1][c-1].getSurroundingBombs() == 0){
                uCS(r+1,c-1);
                
            }
            if(r-1 >= 0 && c+1 < size && board[r-1][c+1].getRevealedStatus() == false && board[r-1][c+1].getSurroundingBombs() == 0){
                uCS(r-1,c+1);
                
            }
            if(r-1 >= 0 && c-1 >= 0 && board[r-1][c-1].getRevealedStatus() == false && board[r-1][c-1].getSurroundingBombs() == 0){
                uCS(r-1,c-1);
