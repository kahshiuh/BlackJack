...
public class MinesweeperFrame extends javax.swing.JFrame implements ActionListener {
            ...
            for (int i = 0; i < model1.getSize(); i++) {
                for (int j = 0; j < model1.getSize(); j++) {
                    ...
                    if (me.getSource() == butArray[i][j]) {
                        //Flags square
                        if (right == true && !shown[i][j] && !model1.getSquare(i,j).getRevealedStatus() && flagsRemaining > 0 && firstClick == false) {
                ...
                if (ae.getSource() == butArray[r][c]) {
                    ...
                    for (int i = 0; i < model1.getSize(); i++) {
                        for (int j = 0; j < model1.getSize(); j++) {
                            ...
                            if(flagged[i][j] && model1.getSquare(i,j).getSurroundingBombs() == 0 && model1.getSquare(i, j).getRevealedStatus()){
                                butArray[i][j].setIcon(numbers[model1.getSquare(i, j).getSurroundingBombs()]);
                           }
    ...
    //scrw you mr hanley iom done with thi opressibve behavior im DONE Done im about to call CPS on you and report you on the GRAPEVINEEEEEE
